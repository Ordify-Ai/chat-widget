...
| `data-ordify-enable-image-generation` | `true` / `1` sets `enableImageGeneration` for client affordances only; the API still requires `allow_image_generation` on the publishable key. |
...
| `enableImageGeneration` | boolean | `false` | **Optional** - Client affordance for future image UI; **server** must allow image generation on the publishable key (`allow_image_generation`). |
